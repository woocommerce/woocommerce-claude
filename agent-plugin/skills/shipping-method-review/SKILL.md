...
This is a historic shipping-method and order-mix review, not a carrier report, arrival-timing audit, capacity plan, profitability analysis, or checkout-performance diagnosis. Use actual returned period figures only. Do not infer unreturned carrier facts, operational capacity, cost base, processor reversals, pre-order activity, or causation unless the merchant supplies that context.
...
5. If shipping-method pipeline is material, fetch order pipeline context:
   ...
   - Use this for overall on-hold order age, status mix, and payment-method pipeline context. Do not describe on-hold orders as a shipping or fulfilment queue.
...
7. Use `wc-analytics-rows` only if the merchant explicitly asks for the specific orders behind a method, a shipping-charge filter, or a pipeline queue. Keep customer details pseudonymised and aggregated by default.
...
- Use the returned shipping-charge total from headline revenue for charges collected. Do not split shipping charges by method unless a returned field provides that split.
...
- Multi-line shipping orders are represented by one returned method label. Do not claim the review is a complete carrier allocation for orders that used multiple shipping lines.
- Pipeline revenue and orders are on-hold payment pipeline, not collected revenue, lost revenue, a fulfilment queue, or an arrival-timing issue. When in doubt, simply say they are awaiting payment.
- Refunds attached to a shipping method are diagnostic signals. They do not prove fulfilment problems, carrier problems, buyer dissatisfaction, processor reversals, or method quality.
...
- Do not claim unreturned carrier results, arrival timing, operational capacity, warehouse constraints, team constraints, cost base, ad economics, pre-order activity, checkout performance, processor reversals, buyer motivations, or causation unless the merchant supplies that context.
- Do not claim a method explains revenue movement, pipeline, refunds, or unassigned coverage. Phrase findings as "worth checking" against shipping settings, payment workflow, fulfilment notes, support tickets, or carrier dashboards the merchant already uses.
...
- Do not suggest building a new skill, endpoint, connector, or plugin feature. Suggest merchant actions available today in WooCommerce admin, shipping zones, shipping classes, shipping method labels, payment workflows, fulfilment/support workflows, carrier dashboards, or connected analytics tools.
...
Report the store-wide shipping charges collected when returned, with comparison movement when returned. Be explicit that this is the available charge figure for the period; do not split charges by method unless returned. Mention tax separately only when useful and returned.
...
List methods with notable refund amount, refund count, or refund rate when returned. Include small-sample caveats. Frame each as a support, fulfilment, or shipping-settings check, not a conclusion about service quality or carrier results.

#### 6. Operational Checks

List two or three checks grounded in the data: shipping-zone labels, free-shipping or local-pickup settings, shipping classes/rates, on-hold payment follow-up, refund/support notes, or carrier-dashboard checks where the merchant already has that dashboard. Do not invent arrival timing or capacity problems.

#### 7. Next Actions

Give three concrete merchant-actionable steps. Each should be doable in WooCommerce admin, shipping settings, shipping classes, payment/order workflows, fulfilment/support workflows, carrier dashboards, or connected analytics tools.
