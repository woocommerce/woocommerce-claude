...
8. Optional follow-ups only when supported by the returned data and the merchant's question:
   ...
   - If the merchant asks whether a named channel, source, or campaign appears in orders for a named country, use `wc-analytics-rows` with `entity: orders`, `mode: aggregate`, filters for that billing country and that named attribution value, and report only the returned aggregate order/revenue result.
   - If the merchant asks whether a named product appears in orders for a named country, use `wc-analytics-rows` with `entity: orders`, `mode: aggregate`, filters for that billing country and product, and report only the returned aggregate order/revenue result.
   - If the merchant asks which products drove a country and no returned data provides that cross-cut, say the current aggregate view does not split products inside a billing country; suggest checking WooCommerce orders with a country filter or inspecting a named product-country pair.
...
- Product and attribution follow-ups are limited. Store-wide product or channel breakdowns do not prove what drove a specific country. Use them only as broader context, or use rows aggregate for a named country/product or country/channel check when the merchant asks a precise question and the filters are supported.
