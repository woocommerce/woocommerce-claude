...
In Claude Code, add the repository as a marketplace with `/plugin marketplace add woocommerce/woocommerce-claude`, then install it with `/plugin install woocommerce-claude@woocommerce-claude-ai-toolkit`. After `/reload-plugins`, try `/woocommerce-claude:weekly-store-review`, `/woocommerce-claude:revenue-drop-triage`, `/woocommerce-claude:failed-order-triage`, `/woocommerce-claude:refund-triage`, `/woocommerce-claude:coupon-performance-triage`, `/woocommerce-claude:customer-value-review`, `/woocommerce-claude:inventory-risk-review`, or `/woocommerce-claude:tax-reconciliation`. Some Claude Code builds also expose the shorter aliases `/weekly-store-review`, `/revenue-drop-triage`, `/failed-order-triage`, `/refund-triage`, `/coupon-performance-triage`, `/customer-value-review`, `/inventory-risk-review`, and `/tax-reconciliation`.
