...
In Claude Code, the agent plugin can be installed Shopify-style with `/plugin marketplace add woocommerce/woocommerce-claude`, then `/plugin install woocommerce-claude@woocommerce-claude-ai-toolkit`. After `/reload-plugins`, try `/woocommerce-claude:weekly-store-review`, `/woocommerce-claude:revenue-drop-triage`, `/woocommerce-claude:failed-order-triage`, `/woocommerce-claude:refund-triage`, `/woocommerce-claude:coupon-performance-triage`, or `/woocommerce-claude:tax-reconciliation`. Some Claude Code builds also expose the shorter aliases `/weekly-store-review`, `/revenue-drop-triage`, `/failed-order-triage`, `/refund-triage`, `/coupon-performance-triage`, and `/tax-reconciliation`.
