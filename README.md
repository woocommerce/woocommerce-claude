...
Plus three resources (`store://profile`, `store://catalog-schema`, `store://policies`) and five prompts (`wc-prompts-catalog-audit`, `wc-prompts-product-improve`, `wc-prompts-weekly-store-review`, `wc-prompts-failed-order-triage`, `wc-prompts-refund-triage`).

The repository also includes reference agent Skills in [`skills/`](./skills/). These are workflow wrappers around the MCP tools — for example, `weekly-store-review` combines revenue, orders, customers, products, attribution, and refunds into one merchant-friendly weekly review, `failed-order-triage` turns order status and payment-pipeline diagnostics into an action queue, and `refund-triage` turns refund size, timing, and product/country drivers into practical checks. The MCP tools stay as stable data primitives; Skills carry the opinionated workflow guidance.
...
In Claude Code, the agent plugin can be installed Shopify-style with `/plugin marketplace add woocommerce/woocommerce-claude`, then `/plugin install woocommerce-claude@woocommerce-claude-ai-toolkit`. After `/reload-plugins`, try `/woocommerce-claude:weekly-store-review`, `/woocommerce-claude:failed-order-triage`, or `/woocommerce-claude:refund-triage`. Some Claude Code builds also expose the shorter aliases `/weekly-store-review`, `/failed-order-triage`, and `/refund-triage`.
