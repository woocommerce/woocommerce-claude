...
Plus three resources (`store://profile`, `store://catalog-schema`, `store://policies`) and six prompts (`wc-prompts-catalog-audit`, `wc-prompts-product-improve`, `wc-prompts-weekly-store-review`, `wc-prompts-revenue-drop-triage`, `wc-prompts-failed-order-triage`, `wc-prompts-refund-triage`).
...
In Claude Code, the agent plugin can be installed Shopify-style with `/plugin marketplace add woocommerce/woocommerce-claude`, then `/plugin install woocommerce-claude@woocommerce-claude-ai-toolkit`. After `/reload-plugins`, try `/woocommerce-claude:weekly-store-review`, `/woocommerce-claude:revenue-drop-triage`, `/woocommerce-claude:failed-order-triage`, or `/woocommerce-claude:refund-triage`. Some Claude Code builds also expose the shorter aliases `/weekly-store-review`, `/revenue-drop-triage`, `/failed-order-triage`, and `/refund-triage`.
