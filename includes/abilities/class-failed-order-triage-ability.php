...
Use period "{$period}" unless the merchant explicitly asked for exact dates. Use the headline order overview with comparison enabled only to get the comparison dates plus status/pipeline diagnostics; do not quote its paid-order or paid-revenue comparison in the final answer.
...
2. Call wc-analytics-totals with subject=orders and compare=true. Read status_breakdown and the pipeline diagnostic. Treat the current paid-order count as optional background only; do not mention whether paid orders grew, shrank, improved, or declined.
...
Two or three sentences covering on-hold orders, failed orders, and whether those buckets are growing, shrinking, or flat. You may mention only the current paid-order count as neutral background, never paid-order growth, a denominator, a share, or a reason to downgrade the issue.
