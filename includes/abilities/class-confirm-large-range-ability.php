	...
	public static function register() {
		wp_register_ability(
			...
			array(
				...
				'description'         => __(
					<<<'DESCRIPTION'
Approves a pending large date range analytics query. Call this after wc-analytics/get-data returns an extended_range_required error, and only after presenting the cost estimate to the merchant and receiving their explicit go-ahead.
...
1. wc-analytics/get-data returns extended_range_required with cost_estimate.
...
4. Call this tool with the same date_start, date_end, and type from the failed get-data call. Include a description so the merchant sees a meaningful label in the approval prompt.
5. Call wc-analytics/get-data again with the same params — the gate will pass.
...
Good (correct): Merchant asks for 3 years. Gate fires. I present options. Merchant says yes. I call this tool. I call wc-analytics/get-data again.
...
The approval window is 5 minutes. If it has expired, call wc-analytics/get-data again to get a fresh cost estimate, present it to the merchant, and wait for their reply.
