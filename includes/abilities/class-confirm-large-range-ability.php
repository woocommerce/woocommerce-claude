	...
	public static function register() {
		wp_register_ability(
			...
			array(
				...
				'description'         => __(
					<<<'DESCRIPTION'
Approves a pending large date range analytics query. Call this after an analytics tool returns an extended_range_required error, and only after presenting the cost estimate to the merchant and receiving their explicit go-ahead.
...
1. An analytics tool returns extended_range_required with cost_estimate.
...
4. Call this tool with the same date_start, date_end, and type or subject from the failed analytics call. Include a description so the merchant sees a meaningful label in the approval prompt.
5. Call the original analytics tool again with the same params — the gate will pass.
...
Good (correct): Merchant asks for 3 years. Gate fires. I present options. Merchant says yes. I call this tool. I call the original analytics tool again.
...
The approval window is 5 minutes. If it has expired, call the original analytics tool again to get a fresh cost estimate, present it to the merchant, and wait for their reply.
...
DESCRIPTION,
		...
		return array(
			...
			'properties' => array(
				'date_start'  => array(
					'type'        => 'string',
					'pattern'     => '^\\d{4}-\\d{2}-\\d{2}$',
					'description' => 'Start date of the query being approved (YYYY-MM-DD). Must match the date_start in the params that triggered the gate.',
				),
				'date_end'    => array(
					'type'        => 'string',
					'pattern'     => '^\\d{4}-\\d{2}-\\d{2}$',
					'description' => 'End date of the query being approved (YYYY-MM-DD). Must match the date_end in the params that triggered the gate.',
				),
				'type'        => array(
					'type'        => 'string',
					'enum'        => array(
					...
					'description' => 'The analytics type or verb-shaped subject being approved.',
				),
				'description' => array(
					'type'        => 'string',
	...
	public static function execute( $input ) {
		...
		if ( ! LargeRangeGate::approve_scan( $date_start, $date_end, $type ) ) {
			return new \WP_Error(
				'no_pending_query',
				'No pending query found for this date range — the approval window may have expired (5 minutes). Call the original analytics tool again to get a fresh cost estimate, present it to the merchant, and wait for their reply.',
				array( 'status' => 400 )
			);
		}
		...
		return array(
			'confirmed' => true,
			'message'   => 'Query approved. Call the original analytics tool again with the same params to retrieve the data.',
		);
	}
}
