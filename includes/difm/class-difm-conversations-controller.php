<?php
/**
 * REST controller for AI Insights conversation persistence.
 *
 * Routes:
 *   GET  /woocommerce-claude/v1/difm/conversations — Return stored conversations for the current user.
 *   POST /woocommerce-claude/v1/difm/conversations — Create or update a conversation (upsert by ID).
 *
 * Conversations are stored as a JSON-encoded array in user meta under the key
 * `woocommerce_claude_conversations`, capped at MAX_CONVERSATIONS entries
...
class DifmConversationsController {
	 ...
	 */
	const USER_META_KEY = 'woocommerce_claude_conversations';
