<?php
/**
 * Workflow skill loader for WooCommerce for Claude AI Insights.
 *
 * The companion agent plugin reads the same per-workflow SKILL.md files directly.
 * AI Insights runs inside WordPress, so it needs a lightweight PHP loader that
...
class WorkflowSkills {
	...
	public static function all() {
		...
		$files           = glob( WOOCOMMERCE_CLAUDE_PLUGIN_DIR . 'skills/*/SKILL.md' );
	...
	/**
	 * Return the Anthropic-ready workflow instructions for AI Insights.
	 *
	...
	public static function prompt_for_difm( array $workflow ) {
		...
		return trim(
			...
			. "The merchant's latest message matches this WooCommerce workflow. Follow these instructions for this turn. Use the available AI Insights tools quietly, and do not mention workflow names, tool names, parameter names, or implementation details in the merchant-facing answer.\n\n"
	...
	private static function select_command_workflow( $message ) {
		if ( 1 !== preg_match( '/^\s*\/(?:woocommerce-claude:)?([a-z0-9-]+)\b/i', (string) $message, $matches ) ) {
			return null;
		}
	...
	/**
	 * Translate MCP/client-facing references to AI Insights tool names.
	 *
	...
	private static function translate_tool_references( $body ) {
		$replacements = array(
			...
			'WooCommerce for Claude analytics tools'     => 'the available AI Insights analytics tools',
			'MCP server\'s extended-range approval flow' => 'server-led extended-range approval flow',
			'MCP server'                                 => 'AI Insights tool bridge',
			'MCP resource'                               => 'tool',
			'MCP'                                        => 'AI Insights',
		);
